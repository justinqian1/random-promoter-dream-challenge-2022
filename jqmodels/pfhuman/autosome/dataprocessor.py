...
class AutosomeDataProcessor(DataProcessor):
    def __init__(
        ...
        self.train = preprocess_df(path=path_to_training_data, seqsize=seqsize)
        if path_to_validation_data is not None:
            self.valid = preprocess_df(path=path_to_validation_data, seqsize=seqsize)
        else:
            self.valid = None
    ...
    def data_channels(self) -> int:
        return 5 # 4 - onehot, 1 - is_reverse
